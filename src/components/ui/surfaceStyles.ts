...
export function getGlassCardStyle(light: LightTheme): CSSProperties {
  return getFrostedPaperCardStyle(light);
}
...
export function getGlassPillStyle(light: LightTheme): CSSProperties {
  return {
    background: `linear-gradient(180deg, rgba(255,255,255,0.40) 0%, rgba(255,255,255,0.12) 100%), ${light.cardBg}`,
    backdropFilter: "blur(18px) saturate(1.14)",
    WebkitBackdropFilter: "blur(18px) saturate(1.14)",
    border: `0.5px solid ${light.glassBorder}`,
    boxShadow: light.glassShadow,
    position: "relative",
    transition: "all 1s ease-in-out",
  };
}
