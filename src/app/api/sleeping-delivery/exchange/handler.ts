...
import {
  ONBOARDING_CHOICE_CAPABILITY,
  ONBOARDING_CHOICE_REQUESTED_COUNT,
  buildOnboardingJourneyDeliveryId,
} from "../../../../lib/server/onboardingDeliveryBundle";
import {
...
async function handleExchangePost(request: Request) {
  ...
  const onboardingJourneyMetadata: Record<string, string> = onboardingJourneyId
    ? {
  ...
  const isOnboardingChoiceCapableRequest = Boolean(
    isOnboardingExchange &&
      !debugDryRun &&
      input.deliveryDateKey &&
      onboardingJourneyId &&
  ...
  const isFourChoiceCapableRequest =
    isEveningChoiceCapableRequest || isOnboardingChoiceCapableRequest;
  ...
  const eveningChoiceAssignedVariant: EveningChoiceVariant =
    isFourChoiceCapableRequest &&
    (isOnboardingChoiceCapableRequest ||
  ...
  if (
    ...
    (isFourChoiceCapableRequest || Boolean(onboardingJourneyId))
  ) {
    const existingBundle = onboardingJourneyId
      ? await readExistingOnboardingDeliveryBundle({
          supabase,
          bundleId: idempotentDeliveryId,
          submissionId: onboardingLedger!.submissionId!,
        })
      : await readExistingDeliveryBundle({
          supabase,
          userId,
          anonymousId,
          bundleId: idempotentDeliveryId,
      ...
      return NextResponse.json({
        ...
        ...(isFourChoiceCapableRequest
          ? {
              ...
              bundleId: idempotentDeliveryId,
              experienceVersion: isOnboardingChoiceCapableRequest
                ? ONBOARDING_CHOICE_CAPABILITY
                : EVENING_CHOICE_CAPABILITY,
  ...
  if (isOnboardingExchange && !debugDryRun) {
    const priorDelivery = await readPriorExchangeForIdentity({
      ...
      userId,
      senderAnonymousId,
      excludeLocalDeliveryIds: idempotentDeliveryId
        ? [
            idempotentDeliveryId,
            ...Array.from({ length: EVENING_CHOICE_MAX_COUNT }, (_, index) =>
  ...
  if (
    ...
    shouldUseEveningChoiceBundlePath({
      isCapableRequest: isFourChoiceCapableRequest,
    ...
    return handleEveningChoiceExchange({
      ...
      experienceVersion: isOnboardingChoiceCapableRequest
        ? ONBOARDING_CHOICE_CAPABILITY
        : EVENING_CHOICE_CAPABILITY,
      onboardingSubmissionId: onboardingLedger?.submissionId ?? null,
      supabase,
      timing,
      userId,
      anonymousId,
      ...
      if (idempotentDeliveryId && isUniqueDeliveryError(deliveryError)) {
        ...
        if (onboardingJourneyId && onboardingLedger?.submissionId) {
          ...
          if (existingBundle.length > 0) {
            ...
            if (responseDeliveries) {
              markExchangeTiming(timing, "read_duplicate_choice_bundle");
              return buildExistingBundleResponse({
                deliveries: responseDeliveries,
                input,
                isEveningChoiceCapableRequest: false,
                supabase,
                timing,
                userId,
                anonymousId,
...
async function handleEveningChoiceExchange({
  ...
  experienceVersion:
    | typeof EVENING_CHOICE_CAPABILITY
    | typeof ONBOARDING_CHOICE_CAPABILITY;
  input: Required<ExchangeRequest>;
  isOnboardingExchange: boolean;
  onboardingSubmissionId: string | null;
  onboardingJourneyMetadata: Record<string, string>;
  ...
  const deliverableRows = remoteRows.filter(
      (row) =>
  ...
  if (selectedCandidates.length === 0) {
    ...
    return NextResponse.json({
      ...
      diagnostics: {
        ...
        choiceExperienceVersion: experienceVersion,
  ...
  const isOnboardingSingleFallback =
    isOnboardingExchange && servedVariant === "single_v1";
  ...
  const prepared = await Promise.all(
    selectedCandidates.map(async (selected, index) => {
      const localDeliveryId = isOnboardingSingleFallback
        ? bundleId
        : buildEveningChoiceDeliverySlotId({
            bundleId,
            position: index + 1,
      ...
      return {
        ...
        row: {
          ...
          recipient_local_cat_id: input.recipientCatId,
          photo_url: deliveryPhotoSrc,
          status: isOnboardingSingleFallback ? "kept" : "delivered",
          metadata,
          delivered_at: deliveredAt.toISOString(),
        },
      };
  ...
  if (deliveryError) {
    if (isUniqueDeliveryError(deliveryError)) {
      const existingBundle =
        isOnboardingExchange && onboardingSubmissionId
          ? await readExistingOnboardingDeliveryBundle({
              supabase,
              bundleId,
      ...
      if (existingBundle.length > 0) {
        if (isOnboardingExchange && onboardingSubmissionId) {
          await transferOnboardingExchangeRows({
            supabase,
            anonymousId,
      ...
      if (isOnboardingExchange && onboardingSubmissionId) {
        const existingSingleDelivery = await readExistingOnboardingDelivery({
          supabase,
          localDeliveryId: bundleId,
          submissionId: onboardingSubmissionId,
        });

        if (existingSingleDelivery) {
          await transferOnboardingExchangeRows({
            supabase,
            anonymousId,
            deliveryId: existingSingleDelivery.local_delivery_id,
            submissionId: onboardingSubmissionId,
            userId,
          });
          await recordOnboardingDeliveryProgress({
            adminSupabase,
            anonymousId,
            delivery: existingSingleDelivery,
            input,
            userId,
          });
          markExchangeTiming(timing, "read_duplicate_single_delivery");
          return buildExistingBundleResponse({
            deliveries: [existingSingleDelivery],
            input,
            isEveningChoiceCapableRequest: true,
            supabase,
            timing,
  ...
  if (isOnboardingSingleFallback && prepared[0]) {
    await recordOnboardingDeliveryProgress({
      adminSupabase,
      anonymousId,
      delivery: prepared[0].row,
      input,
      userId,
    });
  }
...
async function readExistingOnboardingDeliveryBundle({
  supabase,
  bundleId,
  submissionId,
}: {
  ...
  if (error) {
    console.warn(
      "[sleeping-delivery/exchange] existing onboarding bundle lookup failed",
      { code: error.code },
    );
    return [];
  }
...
async function finalizeOnboardingBundleForLegacyClient({
  bundleId,
  deliveries,
  input,
  supabase,
  ...
  const firstVisibleDelivery = deliveries.find(isVisibleDelivery);
  ...
  const { data, error } = await supabase.rpc(
    ...
    {
      p_submission_id: ledger.submissionId,
      p_resume_token_hash: createHash("sha256")
        .update(ledger.resumeToken)
        .digest("hex"),
      p_bundle_id: bundleId,
      p_selected_local_delivery_id: firstVisibleDelivery.local_delivery_id,
    },
  );

  if (error) {
    console.warn(
      "[sleeping-delivery/exchange] legacy onboarding choice finalize failed",
      { code: error.code },
    );
    return null;
  }

  const resolution = (data?.[0] ?? null) as {
    selected_local_delivery_id?: unknown;
  } | null;

  return typeof resolution?.selected_local_delivery_id === "string"
    ? resolution.selected_local_delivery_id
    : null;
}

async function orderOnboardingBundleForLegacyClient({
  bundleId,
  deliveries,
  input,
  supabase,
  ...
  const canonicalDeliveryId =
    await finalizeOnboardingBundleForLegacyClient({
      bundleId,
      deliveries,
      input,
      supabase,
    });
  const canonicalDelivery = deliveries.find(
    (delivery) =>
      delivery.local_delivery_id === canonicalDeliveryId &&
      isVisibleDelivery(delivery),
  );
  ...
  return [
    ...
    ...deliveries.filter(
      (delivery) =>
        delivery.local_delivery_id !== canonicalDelivery.local_delivery_id,
    ),
  ];
}

function isVisibleDelivery(delivery: RemoteDeliveryRow) {
  return delivery.status !== "hidden" && delivery.status !== "reported";
}
...
async function buildExistingBundleResponse({
  ...
  const visibleDeliveries = deliveries.filter(isVisibleDelivery);
  const photos = await Promise.all(
  ...
  const experienceVersion =
    metadata.experience_version === ONBOARDING_CHOICE_CAPABILITY
      ? ONBOARDING_CHOICE_CAPABILITY
      : EVENING_CHOICE_CAPABILITY;
  ...
  const resolution =
    bundleId && experienceVersion === EVENING_CHOICE_CAPABILITY
...
async function readPriorExchangeForIdentity({
  ...
  userId,
  senderAnonymousId,
  excludeLocalDeliveryIds,
}: {
  supabase: SupabaseClient;
  userId: string | null;
  senderAnonymousId: string | null;
  excludeLocalDeliveryIds: string[];
}) {
  if (userId) {
...
async function transferOnboardingExchangeRows({
  ...
  if (deliveryBundleId) {
    const slotIds = Array.from(
      { length: EVENING_CHOICE_MAX_COUNT },
      (_, index) =>
        buildEveningChoiceDeliverySlotId({
