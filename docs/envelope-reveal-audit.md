...
- `src/components/home/homeEnvelopeMotionConfig.ts`
  - ホーム用封筒開封時間。現在のホームは simple reveal で、開封コミットは `HOME_SIMPLE_REVEAL_COMMIT_MS = 180`。旧 `HOME_ENVELOPE_OPEN_MS = 2200` は detailed reveal 用の残値。
...
| 開封中 | `isEnvelopeOpening = true` | simple envelope art、`ENVELOPE_OPEN_MS` | simple reveal では180ms後に `onOpenDelivery()`。detailed reveal 用の `HomeEnvelopeMotionArt` は通常経路では使わない。 |
...
- duration:
  - simple reveal の開封コミットは `HOME_SIMPLE_REVEAL_COMMIT_MS = 180`。
  - v2では開封側を短く保ち、閉じる側に520msの StampPair stow motion を置く。
- easing:
...
1. ホーム開封時間が長い
   - 現在の simple reveal は180ms。旧 detailed reveal の2200msは通常経路では使わない。
...
理由:
- 現在の一番大きな欠点は「写真ロードと開封演出の同期不足」と「開封後の state4 / StampPair 接続不足」です。
...
2. ホームは開封側を180ms simple revealのまま維持し、閉じる側に520msの StampPair stow motionを置く。
