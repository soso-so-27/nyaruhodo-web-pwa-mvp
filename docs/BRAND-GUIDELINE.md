...
## 4. 語彙
2026-07-25変更: うちのこページ上部の大きな「カバー写真」は置かない。「代表写真」の設定も通常UIには置かず、既存の保存データだけを互換性のため保持する。プロフィール先頭には、すでにある写真から自動で選んだ小さな猫カードを置く。この写真は設定項目ではなく、名前・いっしょに暮らした日数・性別／年齢・編集導線をひとまとまりに見せるための表示とする。

うちのこのプロフィールは、入力済み情報を「この子らしさ」→「基本情報（日付と見た目）」→「ケア」の順で常時表示する。空欄の項目・空のグループは表示せず、保存済み情報を総称だけの折りたたみへ隠さない。

2026-08-01変更: うちのこのプロフィールは、まずその子を見分け、思い出すための場所とする。猫名は共通ヘッダーで示し、先頭の猫カードは写真、いっしょに暮らした日数、性別／年齢、編集入口だけをコンパクトにまとめ、その次の「この子らしさ」を主役にする。「この子らしさ」の項目は項目名の下へ内容を置く縦積みとし、長い言葉を固定二列へ押し込まない。入力済みの基本情報とケアのメモは隠さず、その後に静かな参照情報として表示する。

小さな項目名、補足日付、区切り線は紙の背景上で読めるコントラストを保つ。固定の下部ナビゲーションで本文や操作を覆わない。`猫を追加・管理` は一匹のプロフィールではなく複数猫の管理入口として、本文から視覚的に分ける。ケアのメモは飼い主が入力した事実の記録であり、病名、緊急度、予後、治療や投薬を推定する医療診断にはしない。
