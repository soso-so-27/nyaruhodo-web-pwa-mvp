...
- `StoredPhotoImage` 本体は未指定時の `width` / `height` を付与しない。
- `PhotoTile` はサイズ別の intrinsic width / height を明示的に渡している。
- 2026-07-06 に一度 `StoredPhotoImage` 未指定時の `width=512` / `height=512` 既定値を入れたが、縦横比が正方形として扱われて一部写真が拡大表示される副作用があったため撤回した。
...
B-1 により、同一セッション内の重複発行と一覧の逐次発行はかなり抑制された。B-2 により、`PhotoTile` 経由の一覧サムネイルは Storage 側の `width=800, quality=75` を使う。残る体感差は、全画面やホーム主役画像の display 転送量と decode が支配的になりやすい。
...
- `StoredPhotoImage` 本体では未指定 `width` / `height` を補完しない。
- 既存の紙色プレースホルダ、lazy / async decode、first-view の eager / high 指定を維持。
- 一覧系の `PhotoTile` 経由では引き続き aspect / intrinsic size が明示指定される。
...
- thumbnail variant は `width=800`, `quality=75` の transformed signed URL を使う。
...
| 一覧サムネイル | plain display URL | transformed signed URL `width=800, quality=75` |
