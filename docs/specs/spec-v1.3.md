...
参照資料:`docs/STATUS_REPORT.md`(2026-06-12)、`docs/2026-06-11-delivery-incident-postmortem.md`、`docs/PROD-OPERATIONS.md`「配達保証・安全設計の原則」（旧: 事業方針v2.0 §9/§17/§18。原本は未コミットのまま消失し、2026-07-07に同節へ正式収載）
...
| ③ | 配達プール優先順位の配達保証原則（PROD-OPERATIONS）準拠化+重複配達防止 | ローンチブロッカー |
...
## 2.1 方針（PROD-OPERATIONS「配達保証・安全設計の原則」準拠）
...
## 3.1 方針（PROD-OPERATIONS「配達保証・安全設計の原則」準拠）
...
再配達・シードであることはレスポンス・UI・通知のいずれにも表出させない(同原則「再配達・シード非表出」)。
...
`/api/sleeping-delivery/diagnostics` に Tier別候補件数(tier1/tier2/tier3)、重複除外件数、moderation_status別件数を追加する。運用指標「日次供給/需要比」(PROD-OPERATIONS「週次プールヘルスチェック」参照)の観測に使う。
...
- タイトル:`ゆうびんうけを おく`
- 本文:`ホームがめんに ねてるねこを おいておくと、よる8時の ねこだよりを うけとりやすくなります`
  （2026-07-07改訂: 旧「よる8じの おてがみ」は語彙ロック違反のため差し替え。AGENTS.md参照）
- 実行ボタン:`ゆうびんうけを おく`
- スキップ:`あとで`
...
**禁止事項（AGENTS.md「Product principles」の人質回避原則 準拠）:**
