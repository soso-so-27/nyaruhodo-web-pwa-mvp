...
- GoogleログインはSupabase `signInWithOAuth({ provider: "google" })` を使い、callback routeで `exchangeCodeForSession` を実行する。根拠: `src/app/account/create/page.tsx`, `src/app/auth/callback/page.tsx`
...
| `BETA_TESTER_EMAILS` | β参加者の任意補助allowlist（正は `beta_participants`） | `src/lib/betaAccess.ts` |
