...
⚠️ launch前P1: `HomeInput` / `DayCycleIndicator` 周辺で hydration mismatch warning がE2E webServerログに出ています。これはSSR時刻とhydration後のクライアント時刻でホーム状態判定がずれる可能性のサインで、クライアント時計依存と同根です。20時境界付近で表示状態と配達実体が一時的に食い違うリスクがあるため、UI修正とは別タスクで「時刻依存のホーム状態はマウント後に確定し、SSR初期表示はレイアウト安定プレースホルダにする」方針で解消する必要があります。根拠: `src/components/home/HomeInput.tsx`, `src/lib/home/useEveningDelivery.ts`, `src/components/home/HomeDeskModel.tsx`
...
- launch前P1として、`HomeInput` / `DayCycleIndicator` の hydration mismatch warning を解消する必要があります。修正方針は、SSRでは時刻依存の状態UIを確定せず、マウント後にクライアント側で state 判定を確定すること。20時直前/直後と翌朝5時リセット直後のE2Eを追加し、warningが出ないことも検証対象にします。
...
- ⚠️ launch前P1: ホーム状態判定の hydration mismatch warning が残っています。20時境界の信頼性に関わるため、時刻依存stateをhydration後に確定させる修正とテスト追加が必要です。
