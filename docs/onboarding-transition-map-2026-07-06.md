...
> **履歴資料（2026-07-25に現行仕様から除外）**
> 現在はアプリ内ブラウザでも写真選択から4匹の保存／保存しない、ホーム遷移まで
> 同じブラウザ内で完了し、名前入力とGoogleログインを完了条件にしない。
> 現行の遷移は `docs/system-map/screen-flows.md`、判断は `docs/decisions.md` を正とする。

目的: Instagram / LINE / 通常ブラウザ / PWA から来た人が、どこで止まるかを観察した当時のマップ。
