...
flowchart TD
  ...
  P -- "LINE/IG or Googleなし" --> T["つづきのリンク作成"]
...
| 写真保存 | `saving` | 準備中 | なし | envelope / empty | Android/LINE decode失敗、Storage保存失敗、通信失敗 | `onboarding_photo_submitted`, `photo_submitted`, `take_photo`, `photo_upload_error`, `onboarding_delivery_error` |
...
| handoff復元 | `/onboarding/continue` | 復元してホームへ | 復元してホームへ | `/home?handoff=restored` | 使用済み、期限切れ、LINE/IG内で開いた | `onboarding_handoff_restored`, `onboarding_resumed`, `onboarding_handoff_restore_failed` |
...
| 6 | `onboarding_album_prompt_view` | 保存/アカウント導線に到達 |
| 7 | `onboarding_completed` | method=`delivery_confirmed` / `google` / `handoff` / `already_connected` を分ける |
| 8 | 翌日同一identity再訪、または2枚目の `take_photo` | 2日目への橋。standalone/PWA再訪を優先して見る |
...
| P0 | 開封 → 保存方法表示 | `onboarding_delivery_opened` → `onboarding_album_prompt_view` | 続ける意味が弱い、名前入力で詰まる |
| P0 | 保存方法表示 → 完了 | `onboarding_album_prompt_view` → `onboarding_completed` | Google拒否、handoffが分からない、使用済みリンク |
| P0 | handoff作成 → 復元完了 | `onboarding_completed` method=`handoff` → `onboarding_handoff_restored` | LINE/IG内でcontinueを開く、コピーせず離脱、期限切れ |
| P1 | 完了 → 翌日再訪/2枚目 | `onboarding_completed` → step8 | PWA設置されない、20時便の理解不足 |
...
1. iPhone Safari: `/onboarding?src=instagram_bio` → 写真投入 → 1通開封 → Googleログイン → `/cats?onboarding=1`
2. Android Chrome: `/onboarding?src=instagram_bio` → 写真投入 → 1通開封 → Googleなしでhandoff作成 → 復元
